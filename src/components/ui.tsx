...
/** Below this many tiles the whole list is visible at a glance and a search box
...
export function CategoryGrid({
  ...
  const visible = useMemo(() => {
    ...
    const matches = (category: FinanceCategory) =>
      ...
      {showSearch && (
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('cg.search')}
            className={`${inputClass} ps-9 py-2 text-xs`}
          />
          {query && (
            <button
              type="button"
              onClick={() => setQuery('')}
              className="absolute end-2 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-400 flex items-center justify-center"
            >
              <X className="w-3 h-3" />
            </button>
          )}
