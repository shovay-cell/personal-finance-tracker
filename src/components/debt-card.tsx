...
export function DebtCard({
  ...
  const chooseScope = async (scope: PlanEditScope) => {
    const occurrence = scopeTarget;
    setScopeTarget(null);
    if (!occurrence) return;
    if (scope === 'THIS') {
      setOverrideTarget(occurrence);
      return;
    }
    if (scope === 'RULE') {
      setEditingPlan(plan);
    ...
    const newPlan = await splitFixedSchedulePlanFromOccurrence(occurrence.id, {});
              ...
              {occurrence.isPaid ? (
                ...
                <>
                  {totalCount > 1 && (
                    <button
                      type="button"
                      onClick={() => setScopeTarget(occurrence)}
